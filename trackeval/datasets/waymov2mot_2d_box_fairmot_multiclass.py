...
class WaymoV2MotFairMOTMulticlass(_BaseDataset):
    ...
    def __timestamp_rekeying__(self, read_data):
        keys = list(read_data.keys())
        keys.sort()
        for i, k in enumerate(keys):
            new_k = str(i + 1)
            read_data[new_k] = read_data[k]
            del read_data[k]
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        # Re-keying because waymo uses epochs time stamps
        self.__timestamp_rekeying__(read_data)
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                try:
                    ...
                    # New { Expansion of valid classes to be evaluated
                    time_data[:, 6] = 0
                    for cls_name in self.class_list:
                        cls_id = self.class_name_to_class_id[cls_name]
                        time_data[time_data[:,7] == cls_id, 6] = 1 
                    # }
                except ValueError:
                    if is_gt:
                        raise TrackEvalException(
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        #FIXME: Remove me - if self.benchmark == 'MOT20':
